  ...
  def klass_dropdown(klass)
    content_tag(:div, class: "class-header") do
      form_for([:klass, current_klass(klass)], url: klass_redirect_url, method: :get) do |f|
        f.collection_select(:id, current_user.klasses_in_alphabetical_order, :id, :klass_name, {include_blank: "Classes"}, {class: "select-blend"})
      end
    end
  end
  ...
  def lt_dropdown(klass, lt)
    content_tag(:div, class: ["header-select", "lt-header"]) do
      form_for([:learning_target, current_lt(klass, lt)], url: lt_redirect_path(klass), method: :get) do |f|
        f.collection_select(:name, klass.learning_targets, :id, :name, {include_blank: "All Learning Targets", prompt: "Learning Targets"}, {class: ["select-blend", "select-lts"]})
      end
    end
  end
